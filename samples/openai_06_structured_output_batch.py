"""openai_06_structured_output_batch.py - [비전공자 초급] OpenAI 공식 파라미터 기반 Pydantic 배치(Batch) 구조화된 출력 예제

다수의 비정형 문장(여러 개의 주식 댓글 목록)을 단 한 번의 OpenAI SDK(from openai import OpenAI) 요청으로 전달하고,
...
def run_structured_output_batch_sample():
    print_section_header("06. 비전공자용 OpenAI 공식 SDK Pydantic 배치(Batch) 구조화된 출력 예제")
    ...
    print(f"📡 [SDK 클라이언트 배치 요청] {base_url} (모델: {MODEL_NAME})")
    ...
    try:
        ...
        print("\n✅ [SDK Pydantic 배치 검증 객체 일괄 파싱 성공]:")
        print("-" * 65)
        ...
        print(f"❌ [SDK Pydantic 배치 파싱 실패]: {err}")
        return False
