"""openai_04_model_switch.py
================================================================================
[4단계 실습] OpenAI SDK 기반 동적 모델 교체(Model Switching) 가용 모델 전체 실측
================================================================================
학습 목표:
1. OpenAI SDK client.chat.completions.create(model=...) 호출 시 현재 연결된 서버(RTX 3060 / GTX 1070)에 실제 로드된 LLM 대화 모델을 동적 조회(get_available_llm_models)하여 라이브 전송합니다.
...
def main():
    print_section_header("04. OpenAI SDK 동적 모델 변경(Model Switching) 가용 모델 전체 실측 벤치마크")
    ...
    print(f"📝 동일 질문으로 서버 실시간 가용 LLM 모델 {len(TARGET_MODELS)}종 {TARGET_MODELS} SDK 라이브 호출 시작 (max_tokens={config['benchmark_max_tokens']})...\n")
    ...
    for m_id in TARGET_MODELS:
        ...
        print(f"📡 [SDK 모델 전송] '{m_id}' (피크 VRAM: {vram_mb}MB, 추천맥락: {rec_ctx}/최대 {max_ctx}, 스펙 TPS: {bench_tps})")
