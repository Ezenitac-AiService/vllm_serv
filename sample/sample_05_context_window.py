"""sample_05_context_window.py
================================================================================
[5단계 실습] httpx 기반 맥락 토큰 제한(Context Window Limits) 추론 ON vs OFF 2단계 실측
================================================================================
학습 목표:
...
def main():
    print_section_header("05. httpx 토큰 용량 한도(Context Window) 추론 ON vs OFF 2단계 라이브 비교 (RTX 3060 12GB)")

    if not check_server_health(SERVER_HOST, MAIN_PORT, "vllm_serv 메인 API"):
        return

    # -------------------------------------------------------------------------
    # [Part A] 추론 ON (Reasoning ON): 생각 과정이 max_tokens를 소진하는 현상 가시화 [512, 1024, benchmark_max_tokens]
    # -------------------------------------------------------------------------
    part_a_limits = [512, 1024, config["benchmark_max_tokens"]]
    print(f"▶️ [Part A] 추론 ON 환경에서 max_tokens 설정별 생성 토큰 소진 실측 (생각 과정 표출, {part_a_limits})...\n")
    for limit in part_a_limits:
        print(f"⏱️  [Part A: 추론 ON | max_tokens={limit}] 요청 전송 중...")
        payload = {
            "model": MODEL_NAME,
            "messages": [
        ...
        try:
            with get_httpx_client() as client:
                ...
                print(f"\n{clean_content}")
                print_performance_summary(f"Part A: max_tokens={limit} (추론 ON)", t_start, t_end, gen_tokens=gen_tokens, finish_reason=reason)
                print("-" * 65)

        except Exception as err:
            print(f"❌ [Part A max_tokens={limit} 실패]: {err}\n")

    # -------------------------------------------------------------------------
    # [Part B] 추론 OFF (Reasoning OFF): 순수 답변 길이만으로 max_tokens 한도 제어 [128, 512, default_max_tokens]
    # -------------------------------------------------------------------------
    part_b_limits = [128, 512, config["default_max_tokens"]]
    print(f"\n▶️ [Part B] 추론 OFF 환경에서 순수 답변 길이 한도 실측 (NO_THINK_SYSTEM_PROMPT 적용, {part_b_limits})...\n")
    for limit in part_b_limits:
        print(f"⏱️  [Part B: 추론 OFF | max_tokens={limit}] 요청 전송 중...")
        payload = {
            "model": MODEL_NAME,
            "messages": [
        ...
        try:
            with get_httpx_client() as client:
                ...
                print(f"\n💬 [Part B 순수 답변 생성 출력 블록 (max_tokens={limit})]:")
                print("┌" + "─" * 63 + "┐")
                for line in clean_content.splitlines():
                    print(f"│ {line}")
                print("└" + "─" * 63 + "┘")

                print_performance_summary(f"Part B: max_tokens={limit} (추론 OFF)", t_start, t_end, gen_tokens=gen_tokens, finish_reason=reason)
                print("-" * 65)

        except Exception as err:
            print(f"❌ [Part B max_tokens={limit} 실패]: {err}\n")
