"""sample_02_reasoning_control.py
================================================================================
[2단계 실습] httpx 기반 추론 고찰(Reasoning <think>) ON vs OFF 대칭 성능 비교
================================================================================
학습 목표:
...
def main():
    print_section_header("02. httpx 추론 고찰(Reasoning) ON vs OFF 대칭 비교 실습")

    if not check_server_health(SERVER_HOST, MAIN_PORT, "vllm_serv 메인 API"):
        return

    # -------------------------------------------------------------------------
    # [Part A] 추론 ON (Reasoning ON): 생각 과정 시각적 표출 (config["benchmark_max_tokens"] = 2048)
    # -------------------------------------------------------------------------
    print(f"▶️ [Part A] 추론 ON 요청 전송 (생각 과정 표출, max_tokens={config['benchmark_max_tokens']})...")
    payload_a = {
        "model": MODEL_NAME,
        "messages": [
    ...
    try:
        with get_httpx_client() as client:
            ...
            print(f"\n{clean_a}")
            perf_a = print_performance_summary("Part A: 추론 ON", t_start_a, t_end_a, gen_tokens=gen_a, finish_reason=choice_a.get("finish_reason", "stop"))
    ...
    # -------------------------------------------------------------------------
    # [Part B] 추론 OFF (Reasoning OFF): 순수 답변 즉시 수신 (config["no_think_max_tokens"] = 512)
    # -------------------------------------------------------------------------
    print(f"▶️ [Part B] 추론 OFF 요청 전송 (NO_THINK_SYSTEM_PROMPT 적용, max_tokens={config['no_think_max_tokens']})...")
    payload_b = {
        "model": MODEL_NAME,
        "messages": [
    ...
    try:
        with get_httpx_client() as client:
            ...
            print(f"\n💬 [Part B 순수 답변]:\n{clean_b}")
            perf_b = print_performance_summary("Part B: 추론 OFF", t_start_b, t_end_b, gen_tokens=gen_b, finish_reason=choice_b.get("finish_reason", "stop"))
    ...
    print(f"\n💡 [비교 결과 요약]:")
    print(f"   • 추론 OFF 설정으로 응답 속도가 약 {speed_up:.1f}배 향상되었습니다!")
