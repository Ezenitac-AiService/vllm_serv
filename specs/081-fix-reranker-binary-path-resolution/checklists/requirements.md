# 명세서 품질 검증 체크리스트: `llama-server` 네이티브 바이너리 경로 바인딩 및 `/v1/rerank` 404 근본 해결
...
- 실측 증거 기반 분석 완료: `/usr/local/lib/ollama/llama-server` 바이너리 감지 누락으로 인한 파이썬 `llama_cpp.server` 폴백이 `/v1/rerank` 404 원인
