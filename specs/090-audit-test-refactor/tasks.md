...
- [ ] T001 프로젝트 아카이빙 디렉토리 `.legacy/archive_088_sync/` 생성
- [ ] T002 `uv` 환경 및 종속성 패키지 정상 구동 검증 (`uv sync`)
...
- [ ] T003 [P] `specs/090-audit-test-refactor/contracts/cuda_build_api.json` 인터페이스 계약 검증
- [ ] T004 [P] 쉘 공통 믹스인 헤더 스크립트 `scripts/common.sh` 초기화
- [ ] T005 [P] 파이썬 CUDA 환경 탐지 진입점 모듈 `src/utils/cuda_env.py` 초기 스케일 구현
...
- [ ] T006 [P] [US1] 자산 전수 감사 및 `.legacy/` 격리 이동 검증 테스트 작성 (`tests/test_asset_inventory.py`)

### Implementation for User Story 1

- [ ] T007 [P] [US1] 프로젝트 자산 스캐너 및 감사 스크립트 구현 (`scripts/audit_assets.py`)
- [ ] T008 [US1] 감사 스크립트를 실행하여 중복/레거시 파일들을 `.legacy/archive_088_sync/` 디렉토리로 이동 조치 (T006, T007에 의존)
- [ ] T009 [US1] 이관 및 정리 결과 감사 리포트 검증 및 workspace 경로 최적화
...
- [ ] T010 [P] [US2] CUDA GPU 탐지 및 llama.cpp 휠 오프로드 검증 테스트 작성 (`tests/test_cuda_env.py`)
- [ ] T011 [P] [US2] 12종 실습 샘플 스크립트(`sample_01`~`06`, `openai_01`~`06`) 구문 및 동적 바인딩 검증 테스트 작성 (`tests/test_sample_scripts.py`)

### Implementation for User Story 2

- [ ] T012 [US2] `src/utils/cuda_env.py` 내 NVIDIA 드라이버/CUDA/cuDNN 버전을 정밀 탐지하고 Fail-Fast 단정을 수행하는 `CudaEnvironmentProfile` 로직 구현 (T010에 의존)
- [ ] T013 [US2] `scripts/verify_wheel_binary.py` 스크립트를 `src/utils/cuda_env.py` 공통 모듈 참조 방식으로 업데이트 및 GPU 지원 단정 로직 적용
...
- [ ] T014 [P] [US3] `scripts/common.sh` 쉘 공통 믹스인 스크립트에 CUDA, 드라이버, OS 검사 함수 구현
- [ ] T015 [US3] `scripts/setup.sh` 내 중복 탐지 코드를 제거하고 `scripts/common.sh` 믹스인 참조 방식으로 리팩토링 (T014에 의존)
- [ ] T016 [US3] `src/` 및 `scripts/` 내 기타 헬퍼 파일들의 파이썬 호출부를 `src/utils/cuda_env.py` 모듈로 통합 치환
- [ ] T017 [US3] `uv run ruff check src/ tests/` 린터 검사를 통한 코드 스타일 및 중복 코드 검증
...
- [ ] T018 [P] Quickstart 검증 문서 및 프로젝트 설명서 업데이트 (`specs/090-audit-test-refactor/quickstart.md`)
- [ ] T019 [헌장 VII] 프로젝트 전체 회귀 테스트 수트 실행 (`uv run pytest`)
- [ ] T020 [Quickstart 실측] Quickstart 검증 시나리오 1~4단계 전체 수행 및 DoD(DoD-001~003) 달성 최종 확인
